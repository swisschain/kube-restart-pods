...
RN=$(kubectl describe deployment $POD -n $NAMESPACE | grep ReplicaSet | grep -v "NewReplicaSetAvailable\|OldReplicaSets" | awk -F"ReplicaSet" '{print $2}' | sed 's#:##g' | sed 's#"##g' | awk '{print $1}')
echo ReplicaSet $RN
